...
export default function ProductCard({ product, layoutMode }: ProductCardProps) {
  ...
  return (
    ...
    <Link
      ...
      state={{ product }}
      className={`group relative flex overflow-hidden rounded-lg border border-white/10 bg-white/5 transition-all duration-300 hover:-translate-y-0.5 hover:border-[#fcc42c]/45 hover:bg-white/[0.07] ${layoutMode === "grid" ? "flex-col" : "flex-col md:flex-row"
        }`}
    >

      <div
        className={`relative flex shrink-0 items-center justify-center overflow-hidden bg-[#011518] ${layoutMode === "grid" ? "aspect-4/3 w-full p-3" : "aspect-4/3 w-full p-3 md:w-80"
          }`}
